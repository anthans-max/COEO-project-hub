          ...
          <Card className="bg-white">
            {ownerActions.map((action) => {
              ...
              return (
                <div
                  key={action.id}
                  className="flex items-center gap-[10px] px-4 py-[10px] border-b border-border-light last:border-b-0 hover:bg-cream transition-colors"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className={`text-[14px] font-medium leading-[1.45] ${done ? "line-through text-[#C8C0B4]" : "text-text-primary"}`}>
                      {[action.description, action.notes].filter((v) => v && v.trim()).join(" — ")}
                    </div>
                    {action.due_date && (
