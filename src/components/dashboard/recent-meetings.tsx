    ...
    <div className="bg-white border border-border rounded-[10px] overflow-hidden">
      {meetings.length === 0 ? (
        ...
        meetings.map((m) => (
          ...
          >
            <div className="text-[11px] text-text-muted min-w-[48px]">
              {m.date ? formatShortDate(m.date) : "—"}
            </div>
            <div className="text-[12px] text-primary font-medium flex-1 leading-[1.3] truncate">{m.title}</div>
            {m.project_name && (
              <span className="text-[10px] px-2 py-0.5 rounded-[8px] bg-cream text-text-muted whitespace-nowrap shrink-0">
                {m.project_name}
              </span>
            )}
          </Link>
